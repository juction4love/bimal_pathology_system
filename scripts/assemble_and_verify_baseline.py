...
print("Assembling complete 00001_bimal_pathology_clean_baseline.sql...")
...
for f in legacy_files:
    ...
    # Match CREATE [OR REPLACE] FUNCTION [public.]func_name(...) RETURNS ... AS $$ ... $$;
    matches = re.finditer(
        r'(CREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+(?:public\.)?([a-zA-Z0-9_]+)\s*\([\s\S]*?\)\s*RETURNS[\s\S]*?(?:AS\s*\$\$[\s\S]*?\$\$|\$func\$[\s\S]*?\$func\$|\$body\$[\s\S]*?\$body\$)[\s\S]*?;)',
        content,
        re.IGNORECASE
    )
    for m in matches:
...
# Locate where the seed data starts
...
# Remove any previous function blocks from header_and_tables to avoid duplication
func_start = header_and_tables.find('-- ============================================================================\n-- 5. BUSINESS FUNCTIONS')
if func_start != -1:
    header_and_tables = header_and_tables[:func_start]

# Also let's ensure all extra operational tables are present
extra_tables = """
-- ============================================================================
...
for name in sorted(functions_map.keys()):
...
full_sql = f"{header_and_tables}\n{extra_tables}\n{functions_sql}\n{master_seed}"

# Ensure BEGIN and COMMIT
if not full_sql.strip().startswith('BEGIN;'):
    full_sql = f"BEGIN;\n\n{full_sql}"
if not full_sql.strip().endswith('COMMIT;'):
    full_sql = f"{full_sql}\n\nCOMMIT;\n"

open('supabase/migrations/00001_bimal_pathology_clean_baseline.sql', 'w', encoding='utf-8').write(full_sql)
print("Successfully generated complete 00001_bimal_pathology_clean_baseline.sql!")
