...
const billingSql = read('supabase/migrations/00018_outsource_sample_tracking.sql');
const idempotencySql = read('supabase/migrations/00043_payment_receivables_integrity.sql');
const traceabilitySql = read('supabase/migrations/00055_foundation_rpc_and_sample_traceability_contracts.sql');
const resultSql = read('supabase/migrations/00056_foundation_result_readiness_and_revision.sql');
const lineageSql = read('supabase/migrations/00021_final_flow_integrity_and_concurrency.sql');
const labNoSql = read('supabase/migrations/00054_shared_catalogue_collection_and_bpdc_foundation.sql');
...
test('patient deduplication is locked and protected by normalized mobile uniqueness', () => {
  ...
  assert.match(read('supabase/migrations/00013_enforce_patient_mobile_uniqueness.sql'), /UNIQUE INDEX IF NOT EXISTS uq_patients_mobile/);
...
test('verification, sign-off, and version lineage remain order-local', () => {
  ...
  assert.match(read('supabase/migrations/00005_reporting_signoff_and_pdf.sql'), /WHERE coi\.order_id = p_order_id/);
});
