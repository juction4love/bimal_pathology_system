"""
Authoritative Clean Production Baseline Generator
...
# Read clean master seed data
# In 00001_bimal_pathology_clean_baseline.sql, extract tests, parameters, reference_ranges, catalogue_panel_components
cur = open('supabase/migrations/00001_bimal_pathology_clean_baseline.sql', 'r', encoding='utf-8').read()

def extract_insert_block(text, table_name):
    pattern = re.compile(rf'INSERT INTO public\.{table_name}[\s\S]*?;', re.IGNORECASE)
    match = pattern.search(text)
    return match.group(0) if match else None

analyzers_insert = extract_insert_block(cur, 'analyzers')
categories_insert = extract_insert_block(cur, 'test_categories')
tests_insert = extract_insert_block(cur, 'tests')
parameters_insert = extract_insert_block(cur, 'parameters')
ranges_insert = extract_insert_block(cur, 'reference_ranges')
components_insert = extract_insert_block(cur, 'catalogue_panel_components')

# Extract verified analyzer rows
councellRows = [
...
# Base DDL Header (Extensions, Enums, Tables)
base_ddl_header = cur[:cur.find('-- ============================================================================\n-- 5. BUSINESS FUNCTIONS')].strip()
...
{analyzer_mappings_insert}
"""

full_baseline_sql = f"{base_ddl_header}\n\n{functions_sql}\n\n{master_seed_section}\n\nCOMMIT;\n"

if not full_baseline_sql.startswith('BEGIN;'):
    full_baseline_sql = f"BEGIN;\n\n{full_baseline_sql}"

open('supabase/migrations/00001_bimal_pathology_clean_baseline.sql', 'w', encoding='utf-8').write(full_baseline_sql)
print("Authoritative clean baseline written successfully!")
