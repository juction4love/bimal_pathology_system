...
FROM public.tests t
WHERE 
  t.department ILIKE '%Coagulation%' 
  OR t.department ILIKE '%Hematology%'
  OR t.department ILIKE '%Haematology%'
  OR t.name ILIKE '%Bleed%'
  OR t.name ILIKE '%Clot%'
  OR t.name ILIKE '%Time%'
  OR t.name ILIKE '%Prothrombin%'
  OR t.code ILIKE '%COA%'
  OR t.code ILIKE '%HEM%'
  OR t.code ILIKE '%HAE%'
  OR t.short_name IN ('PT', 'BT', 'CT', 'APTT', 'INR')
ORDER BY t.department, t.code;
`;

const tmpFile = path.resolve('tmp_inspect_coag.sql');
...
try {
  ...
  writeFileSync('scripts/inspect_coag_results.json', JSON.stringify(results, null, 2), 'utf8');
  console.log(`Found ${results?.length || 0} tests in Coagulation/Hematology/Time/etc.`);
