-- PostgreSQL ARE rejects repetition bounds above 255. The previous
-- previous counted-repetition validator therefore raised SQLSTATE 2201B for every
-- report notification. Validate the alphabet and length separately.
...
-- Preserve sign-off isolation but retain safe failure evidence for operations.
...
DECLARE
    ...
    v_notification_sqlstate TEXT;
BEGIN
    v_report_result := public.sign_and_freeze_diagnostic_report(
        p_order_id, p_performed_by_id, p_signed_by_id,
        p_amendment_reason, p_amended_from_report_id
    );

    BEGIN
        v_notification_result := public.create_public_report_token(
            (v_report_result->>'report_id')::UUID,
            p_token_hash, 30, p_public_report_url
        );
    EXCEPTION WHEN OTHERS THEN
        ...
        v_notification_result := jsonb_build_object(
            ...
            'error_code', v_notification_sqlstate
        );
    END;
    ...
    IF v_notification_sqlstate IS NOT NULL THEN
        INSERT INTO public.audit_logs(user_id, user_name, action, entity_type, entity_id, new_data)
        VALUES (
            auth.uid(), 'Authorized Signatory', 'REPORT_NOTIFICATION_FAILED',
            'DiagnosticReport', v_report_result->>'report_number',
            jsonb_build_object(
                'report_id', v_report_result->>'report_id',
                'error_code', v_notification_sqlstate
            )
        );
    END IF;
...
DROP FUNCTION public.diagnose_report_notification_rollback(UUID, UUID);
