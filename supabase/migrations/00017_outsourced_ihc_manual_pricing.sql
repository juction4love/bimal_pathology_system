...
-- Migration 00017: Outsourced IHC Manual Pricing & Bill Item Description Support
-- Adds allow_manual_price to master catalogue, item_description to bill_items,
-- seeds master IHC investigation (NoReporting, default price 0, manual price TRUE),
-- and extends create_patient_bill_and_order RPC with strict server-side validation.
-- ============================================================================
...
-- 2. Seed / Upsert Master IHC Investigation
INSERT INTO public.tests (
    ...
    120,
    'Outsourced Immunohistochemistry panel reported by referral laboratory.',
    TRUE,
    TRUE,
    999
)
...
-- 3. Extend Authoritative Billing RPC
...
BEGIN
    ...
    -- ------------------------------------------------------------------------
    -- 2. SERVER-SIDE FINANCIAL MATH (Integer Paisa - Strict Manual Rate Security)
    -- ------------------------------------------------------------------------
    v_gross_paisa := 0;
    
    FOREACH v_item IN ARRAY p_items_data LOOP
        SELECT id, code, name, price_paisa, reporting_type, outsource_lab_name, sample_type, container, allow_manual_price
        INTO v_test
        FROM public.tests
        ...
        -- Strict pricing rule:
        -- If test allows manual pricing (e.g. IHC), operator can provide non-negative manual rate.
        -- Otherwise, server strictly enforces master catalogue price_paisa (rejecting client tampering).
    ...
    -- Insert Bill Items with validated rates & optional descriptions
    FOREACH v_item IN ARRAY p_items_data LOOP
        SELECT id, code, name, price_paisa, reporting_type, outsource_lab_name, sample_type, container, allow_manual_price
        INTO v_test
        FROM public.tests
    ...
    -- ------------------------------------------------------------------------
    -- 4. CLINICAL ORDER & SAMPLE ACCESSIONING (STRICTLY SKIPPED FOR NoReporting ITEMS)
    -- ------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.create_patient_bill_and_order IS 'Authoritative billing RPC with 1 Mobile = 1 Patient, manual pricing support for allowed items (IHC), server financial math, and clinical order routing';
